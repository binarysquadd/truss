...
The presign-upload endpoint lives on the dashboard API (`/api/*`), which is authenticated by the dashboard **session cookie** — not an API key. It is not part of the API-key `/v1` surface, so this example only works from a browser session that is already signed in to the Truss dashboard (the request must carry the session cookie). It is a dashboard-only flow, not a server-to-server one.

```typescript
// 1. Get a presigned upload URL (dashboard session required — sends the session cookie)
