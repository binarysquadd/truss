...
When Kratos is configured and `TRUSS_BOOTSTRAP_ADMIN` is not `false`, Truss seeds a default
...
Metrics (`/metrics`) and structured JSON logs are always on. Tracing is opt-in. See the
[Observability guide](../../guides/observability/) for the full setup.
